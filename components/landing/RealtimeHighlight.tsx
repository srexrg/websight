...
export default function RealtimeHighlight() {
  return (
    <section className="bg-card/50 border-t border-border border-b">
      <div className="max-w-[1180px] mx-auto px-7 py-[84px]">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-[56px] items-center">
          ...
          <div className="bg-card border border-border rounded-2xl p-5 shadow-[0_10px_36px_-16px_rgba(16,24,40,0.2)]">
            {/* Card header */}
            ...
            {liveFeed.map((item, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex items-center gap-[11px] py-[10px] border-b border-border/60"
                >
                  {/* Icon chip */}
                  <div
                    className="w-[30px] h-[30px] rounded-lg flex items-center justify-center flex-shrink-0"
                    style={{ backgroundColor: `${item.fg}1F` }}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11.5px] text-muted-foreground/80">{item.meta}</div>
                  </div>

                  {/* Timestamp */}
                  <span className="font-mono text-[11px] text-muted-foreground/60">{item.ago}</span>
                </div>
              );
            })}
          </div>
          ...
          <div>
            ...
            <Link
              href="/auth"
              className="inline-flex items-center gap-2 bg-[#0E1310] hover:bg-brand text-[#EAF6EF] text-[15px] font-semibold px-5 py-3 rounded-[11px] ring-1 ring-white/10 transition-colors duration-200"
            >
